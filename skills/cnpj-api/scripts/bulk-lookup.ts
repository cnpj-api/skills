...
const BASE_URL = "https://api.cnpj-api.com/v1";
const CHUNK_SIZE = 20;
const MAX_RETRIES = 5;
...
async function callBulk(
  ...
  if (res.status === 429) {
    ...
    process.stderr.write(
      `[bulk-lookup] 429. Waiting ${retryAfterSec}s (attempt ${attempt + 1})\n`,
    );
...
async function main(): Promise<void> {
  ...
  const cnpjs = Array.from(
    new Set(raw.map(clean).filter((c) => c.length === 14)),
  );
  if (cnpjs.length === 0) {
  ...
  process.stderr.write(
    `[bulk-lookup] processing ${cnpjs.length} CNPJs in chunks of ${CHUNK_SIZE}\n`,
  );
